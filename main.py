...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--size-precompute-fill",
        type=int,
        choices=[0, 1],
        default=0,
        help="Precompute limited fill estimates during size tracking to reduce event-finalization delay.",
    )
    parser.add_argument(
        "--size-precompute-max",
        type=int,
        default=3,
        help="Maximum fill candidates to precompute per size track.",
    )
    parser.add_argument(
        "--size-precompute-gap",
        type=int,
        default=10,
        help="Minimum frame gap between precomputed fill evaluations for the same size track.",
    )
    parser.add_argument(
        "--size-keep-candidate-frames",
        type=int,
        choices=[0, 1],
        default=0,
        help="Keep all size candidate frame images. Default 0 deletes non-winning size frames after each event.",
    )
    parser.add_argument(
        "--size-trigger-fill",
        type=int,
        choices=[0, 1],
        default=1,
        help="Prefer fill candidates captured when the truck bbox bottom reaches the trigger zone.",
    )
    parser.add_argument(
        "--size-trigger-bottom-ratio",
        type=float,
        default=0.95,
        help="Start bottom-trigger fill candidates when truck bbox bottom reaches this frame-height ratio.",
    )
    parser.add_argument(
        "--size-trigger-max-candidates",
        type=int,
        default=8,
        help="Maximum bottom-trigger fill candidates per size track.",
    )
    parser.add_argument(
        "--size-trigger-gap",
        type=int,
        default=5,
        help="Minimum frame gap between bottom-trigger fill candidates for the same size track.",
    )
    parser.add_argument(
        "--size-trigger-skip-candidates",
        type=int,
        default=4,
        help="Number of earliest bottom-trigger hits to ignore before collecting size candidates.",
    )
    parser.add_argument(
        "--track-confirm-hits",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default is 0 to avoid extra visualization-only segmentation calls."
        ),
    )
    parser.add_argument(
...
def main() -> int:
    ...
    if args.mode == "stream_event":
        ...
        ok = run_stream_event(
            ...
            size_precompute_fill=bool(args.size_precompute_fill),
            ...
            size_trigger_gap_frames=args.size_trigger_gap,
            size_trigger_skip_candidates=args.size_trigger_skip_candidates,
            ...
            size_show_preview=bool(args.size_show),
