...
def _match_size_event(
    candidate_event: dict[str, Any],
    size_events: list[SizeEvent],
    used_indices: set[int],
    *,
    allow_non_trigger_methods: bool = True,
    best_frame_tolerance: int = 12,
) -> SizeEvent | None:
    ...
    for index, size_event in enumerate(size_events):
        if index in used_indices:
            continue
        if not allow_non_trigger_methods and size_event.fill_selection_method not in {"bottom_trigger", "bottom_trigger_failed"}:
            continue
        if size_event.best_frame < (start_frame - best_frame_tolerance) or size_event.best_frame > (end_frame + best_frame_tolerance):
            continue
...
def _merge_size_result_into_event(event: dict[str, Any], size_event: SizeEvent | None) -> dict[str, Any]:
    ...
    merged["fill_trigger_frames"] = [int(x) for x in size_event.trigger_frames]
    merged["fill_selection_method"] = size_event.fill_selection_method
...
def _emit_event(event: dict[str, Any], events_jsonl_path: Path, logger) -> None:
    ...
    fill_selection_method = str(event.get("fill_selection_method") or "")
    ...
    event_line = _format_event_console_line(
        ...
        fill_best_frame=fill_best_frame,
    )
...
def run_stream_event(
    ...
    size_sampling_fps: float = 5.0,
    size_preview_every_samples: int = 1,
    size_precompute_fill: bool = False,
    ...
    size_trigger_gap_frames: int = 5,
    size_trigger_skip_candidates: int = 4,
    ...
    size_precompute_fill = bool(size_precompute_fill)
    ...
    try:
        ...
        size_pipeline = OnlineSizeEventPipeline(
            ...
            precompute_fill=size_precompute_fill,
            ...
            trigger_gap_frames=size_trigger_gap_frames,
            trigger_skip_candidates=size_trigger_skip_candidates,
        )
    ...
    pending_size_merge_events: list[dict[str, Any]] = []

    def remember_emitted_event(emitted_event: dict[str, Any]) -> None:
        nonlocal events_finalized, heavy_inference_calls_total
        ...
        events_finalized += 1
        heavy_calls = int(emitted_event.get("heavy_inference_calls", 0))
        heavy_inference_calls_total += heavy_calls
        phase_calls_by_event.append(heavy_calls)

    def try_emit_event_with_size_result(event: dict[str, Any], *, final_flush: bool = False) -> dict[str, Any] | None:
        if size_pipeline is not None:
            size_events.extend(size_pipeline.pop_completed_events())
        size_event = _match_size_event(
            event,
            size_events,
            used_size_indices,
            allow_non_trigger_methods=final_flush,
        )
        ...
        if not should_emit:
            logger.info(
                "Dropping finalized event_id=%s before emit (%s)",
                merged_event.get("event_id"),
                drop_reason,
            )
            return {}
    ...
    def emit_event_with_current_size_result(event: dict[str, Any], *, final_flush: bool = False) -> bool:
        emitted_event = try_emit_event_with_size_result(event, final_flush=final_flush)
        if emitted_event:
            remember_emitted_event(emitted_event)
            return True
        if emitted_event == {}:
            return True
        pending_size_merge_events.append(event)
        logger.info(
            "Delaying event_id=%s until size bottom-trigger is ready (event_span=%s-%s best_frame=%s)",
            event.get("event_id"),
    ...
    def emit_ready_pending_size_events(*, final_flush: bool = False) -> None:
        if not pending_size_merge_events:
            return
        remaining: list[dict[str, Any]] = []
        for pending_event in pending_size_merge_events:
            emitted_event = try_emit_event_with_size_result(pending_event, final_flush=final_flush)
            if emitted_event is None:
                remaining.append(pending_event)
                continue
            remember_emitted_event(emitted_event)
        pending_size_merge_events[:] = remaining
    ...
    try:
        while True:
            ...
            if size_pipeline is not None:
                ...
                emit_ready_pending_size_events(final_flush=False)
            ...
            for track in finalized_tracks:
                ...
                if last_frame_shape is not None and int(track.truck_box_xyxy[3]) < int(last_frame_shape[0] * 0.5):
                    logger.info(
                        "Dropping upper-half-ending track %s (truck_bottom=%s, frame_half=%s)",
                        track.track_id,
                        int(track.truck_box_xyxy[3]),
                        int(last_frame_shape[0] * 0.5),
                    )
                    continue
                ...
                if dedup_target is not None:
                    ...
                    continue
                emit_event_with_current_size_result(event)
        ...
        if size_pipeline is not None:
            size_events.extend(size_pipeline.finalize_all())
        emit_ready_pending_size_events(final_flush=True)
